...
import (
	...
	jsonGroup "app/services/contact/internal/delivery/http/group"
	domainGroup "app/services/contact/internal/domain/group"
...
func (d *Delivery) CreateGroup(c *gin.Context) {
	...
	newGroup, err := d.ucGroup.Create(ctx, domainGroup.NewCreator(
		groupName,
		groupDescription,
	))
	if err != nil {
...
func (d *Delivery) UpdateGroup(c *gin.Context) {
	...
	response, err := d.ucGroup.Update(ctx, domainGroup.NewUpdater(
...
func (d *Delivery) DeleteGroup(c *gin.Context) {
	...
	if err := d.ucGroup.Delete(ctx, domainGroup.NewDeleter(converter.StringToUUID(id.Value))); err != nil {
		SetError(c, http.StatusInternalServerError, err)
		return
	}
